...
func (s *ShoppingService) GenerateShoppingListFromDateRange(c echo.Context, name string, startDate, endDate time.Time) (*models.ShoppingList, error) {
	// Create shopping list
	shoppingList, err := s.CreateShoppingList(c.Request().Context(), name, startDate, endDate)
	if err != nil {
		return nil, err
	}

	// Get schedules in date range
	schedules, err := s.scheduleService.GetSchedulesForRange(c, &startDate, &endDate)
	if err != nil {
		return nil, err
	}
	...
	for _, schedule := range schedules {
		err = s.AddMealToShoppingList(c.Request().Context(), shoppingList.ID, schedule.ID)
		if err != nil {
	...
	err = s.RegenerateShoppingListItems(c.Request().Context(), shoppingList.ID)
	if err != nil {
		return nil, err
	}

	// Get the fully populated shopping list
	return s.GetShoppingListById(c.Request().Context(), shoppingList.ID)
}
